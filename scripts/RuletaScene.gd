...
@export var min_impulse_force: float = 45.0 
...
@export var lever_max_angle: float = 55.0    
...
@export var speed_glow_color: Color = Color(1.2, 1.2, 1.3) # Brillo azulado al girar rápido
...
func release_lever():
	...
	if percentage_pulled >= activation_threshold:
		...
		var t = create_tween()
		t.set_parallel(true)
		t.tween_property(lever_sprite, "rotation_degrees", -15.0, 0.1).set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_IN)
		t.chain().tween_property(lever_sprite, "rotation_degrees", 0.0, 0.5).set_trans(Tween.TRANS_ELASTIC).set_ease(Tween.EASE_OUT)
		t.parallel().tween_property(lever_sprite, "modulate", Color.WHITE, 0.3)
		t.parallel().tween_property(lever_sprite, "scale", Vector2.ONE, 0.3).set_trans(Tween.TRANS_ELASTIC)
	else:
		# --- CANCELAR ---
		var t = create_tween()
		t.set_parallel(true)
		t.tween_property(lever_sprite, "rotation_degrees", 0.0, 0.3).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
		t.tween_property(lever_sprite, "modulate", Color.WHITE, 0.2)
		t.tween_property(lever_sprite, "scale", Vector2.ONE, 0.2)
